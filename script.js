...
// === قاعدة بيانات الخدمات الكاملة (أكثر من 500 سطر وتفاصيل المنصات) ===
const servicesData = {
  free: [
    ...
    { id: 105, title: "ملاحظات تعليقات تيك توك تجريبية 🎁", price: 0.0000, speed: "فوري ⚡" },
  ...
  instagram: Array.from({length: 22}, (_, i) => ({
    id: 200 + i,
    title: `إنستغرام - ${['متابعين ضمان 30 يوم', 'إعجابات حقيقية ونشطة', 'مشاهدات ريلز خيالية', 'تفاعلات ستوري VIP', 'تعليقات خليجية وعربية', 'متابعين عربي ضمان', 'لايكات تعليقات انستغرام', 'مشاهدات بث مباشر انستغرام'][i % 8]} #${i + 1}`,
    price: (0.05 + (i * 0.03)).toFixed(4),
    speed: `${(i % 5) + 1} دقائق`
  })),
  tiktok: Array.from({length: 20}, (_, i) => ({
    id: 300 + i,
    title: `تيك توك - ${['مشاهدات فيديوهات سريعة', 'تكبيسات بث مباشر حية', 'إعجابات فيديوهات قوية', 'متابعين حسابات حقيقيين', 'تعليقات عشوائية متفاعلة', 'مشاركات فيديو تيك توك'][i % 6]} #${i + 1}`,
    price: (0.01 + (i * 0.04)).toFixed(4),
    speed: `${(i % 3) + 1} دقائق`
  })),
  telegram: Array.from({length: 50}, (_, i) => ({
    id: 400 + i,
    title: `تليجرام - ${['أعضاء بريميوم مميزين', 'مشاهدات ستوري وقصص', 'تفاعلات إيجابية 👍❤️', 'أعضاء قنوات حقيقيين 100%', 'تصويت استطلاع رأي', 'تعليقات على المنشورات', 'أعضاء ضمان عدم النقصان'][i % 7]} #${i + 1}`,
    price: (0.02 + (i * 0.05)).toFixed(4),
    speed: `${(i % 10) + 1} دقائق`
  })),
  facebook: Array.from({length: 14}, (_, i) => ({
    id: 500 + i,
    title: `فيسبوك - ${['متابعين صفحات وحسابات شخصية', 'لايكات منشورات وصور', 'مشاهدات فيديو فيسبوك', 'تفاعلات ريلز فيسبوك نشطة', 'أعضاء جروبات فيسبوك'][i % 5]} #${i + 1}`,
  ...
  funding: [
    ...
    { id: 603, title: "تمويل مجموعات تليجرام أعضاء متفاعلين", price: 10.0000, speed: "10 ساعات" }
  ],
  youtube: [
    ...
    { id: 703, title: "لايكات فيديوهات يوتيوب حقيقية", price: 2.0000, speed: "30 دقيقة" }
  ],
  twitter: Array.from({length: 5}, (_, i) => ({
    id: 800 + i,
    title: `تويتر (X) - ${['متابعين حقيقيين نشطين', 'إعادة تغريد Retweet', 'إعجابات تغريدات وخوارزميات'][i % 3]} #${i + 1}`,
  ...
  twitch: Array.from({length: 3}, (_, i) => ({
    id: 1000 + i,
    title: `تويتش - ${['متابعين قناة تويتش', 'مشاهدات بث مباشر تويتش'][i % 2]} #${i + 1}`,
...
// جلب بيانات المستخدم من التليجرام أو افتراضي في حال الاختبار
...
// --- لوحة الأدمن الشاملة (تظهر للأدمن فقط) ---
...
if (currentUser && Number(currentUser.id) === ADMIN_ID) {
  ...
  adminPanelContainer.innerHTML = `
    <div style="background: linear-gradient(135deg, #1f1c2c, #393154); border: 2px solid #ff416c; border-radius: 16px; padding: 16px; margin: 15px 0; color: #fff; box-shadow: 0 8px 25px rgba(255,65,108,0.3); direction: rtl; text-align: right;">
      <div style="font-weight: bold; font-size: 15px; margin-bottom: 12px; color: #ff758c;">🛠 لوحة تحكم الأدمن الشاملة</div>
      
      <div style="margin-bottom: 10px;">
        <input type="number" id="adminTargetId" placeholder="آيدي الزبون..." style="width: 100%; padding: 9px; border-radius: 6px; border: none; background: rgba(255,255,255,0.1); color: #fff; margin-bottom: 6px; font-size: 12px; text-align: right;">
        <input type="number" id="adminAmount" placeholder="المبلغ ($) (مثال: 5 أو -5 للخصم)" step="0.01" style="width: 100%; padding: 9px; border-radius: 6px; border: none; background: rgba(255,255,255,0.1); color: #fff; margin-bottom: 8px; font-size: 12px; text-align: right;">
      ...
      <div style="margin-top: 12px; border-top: 1px solid rgba(255,255,255,0.1); padding-top: 10px;">
        <div style="display: flex; gap: 6px; margin-bottom: 8px;">
          <input type="number" id="logTargetId" placeholder="آيدي الزبون لعرض سجله..." style="flex: 1; padding: 9px; border-radius: 6px; border: none; background: rgba(255,255,255,0.1); color: #fff; font-size: 12px; text-align: right;">
...
// --- بناء صفحة الملف الشخصي (البروفايل) الفخمة والمطلوبة بدقة ---
function buildProfileTab() {
  ...
  // ربط زر البروفايل تماماً [بجانب زر الإشعارات] في الشريط السفلي بناءً على طلبك بالحرف الواحد
  const navItems = document.querySelectorAll('.nav-item');
  if (navItems.length >= 5) {
    // الزر الذي بجانب الإشعارات تماماً في الشريط السفلي
    const profileTriggerBtn = navItems[0]; // زر المزيد أو الزر المجاور للإشعارات
    if (profileTriggerBtn) {
      profileTriggerBtn.onclick = (e) => {
        e.preventDefault();
